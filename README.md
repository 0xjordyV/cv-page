# 🧬 Jorge Luis Villalobos Medrano — Portfolio

Welcome to my professional narrative. This space is a journey through my evolution as an engineer, from architecting decentralized ecosystems to building the future of AI-driven automation.

Explore a career defined by high-impact technical leadership, where blockchain innovation meets intelligent agentic workflows to solve complex modern challenges.

## 🚀 Key Professional Pillars

### **AI Automation & Data Engineering**
Specializing in agentic workflows, multi-agent systems, and Python-based automation. Currently driving data excellence and process optimization at **INEGI** (Mexico's National Institute of Statistics and Geography).

### **Web3 & Blockchain Architecture**
Former **CIO at Vibing Studios**, where I led technical strategy and development for high-scale Solana projects, including NFT engines, DeFi activations ($BONK), and gaming infrastructure (Bitmon).

### **Digital Innovation**
A track record of taking complex ideas from concept to deployment, focusing on performance, scalability, and seamless user experiences.

## 📈 Featured Projects
* **INEGI / Enlace Informático:** Modernizing data workflows through AI and advanced analysis.
* **$BONK Activation:** High-traffic decentralized application with over 30,000 real-time interactions.
* **Monkettes & Bitmon:** End-to-end architecture for digital collections and Play-to-Earn gaming ecosystems.
* **Automation Consulting:** Helping businesses scale through custom-built intelligent agents.

## 📩 Connect & Collaborate
I am always open to discussing new technologies, remote Web3 opportunities, or AI automation projects.

* **LinkedIn:** [linkedin.com/in/jorgeluis009](https://linkedin.com/in/jorgeluis009)
* **X (Twitter):** [@0xJordyV](https://x.com/0xJordyV)
* **GitHub:** [0xjarz](https://github.com/0xjarz)
* **Email:** jmedrano0929@gmail.com

---
*"The best way to predict the future is to automate it."*